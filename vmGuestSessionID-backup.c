 ...
 *
 * NOTE: This code was modified from the vmGuestLibTest.c baseline.
 *
 */
...
void getSessionAlert(char *FromFile,uint64_t localId )
{
	...
	sprintf(bufferId,"0x%"FMT64"x\n",localId);
        ...
        save_output = fopen("/var/tmp/vm_session_id-copy.txt","a+");
        fprintf(save_output, "=============================================================================\n", draw_line);
	fprintf(save_output, "Session Reading Time: %s", ctime (&rawtime) );
        ...
        {
          fprintf(save_output, "Session ID remains the same. No vMotion activity has taken place.\n", draw_line);
        }
        else
        {
